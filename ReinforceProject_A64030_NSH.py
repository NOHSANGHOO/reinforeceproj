##################################################################################################### 
# https://github.com/NOHSANGHOO/reinforeceproj
...
#####################################################################################################
# <MAB관련 CLASS 정의>
# BanditEnv : Bandit Environment로 이전 action & score & 이전 score를 통해 reword를 계산한다.
#             socore(모델성능)이 개선되었다면 reword 1 발생한다.
# BanditArmed : Bandit Environment에 reword list를 추가한 Env
# thomsonArmed : Thompson Sampling 기반의 MAB 학습 CALSS
# softArmed    : Softmax Exploration 기반의 MAB 학습 CALSS
...
# Data File List Check(number of file have to be 11)
...
# PART2 :Data LOAD & 전처리 #########################################################################
#####################################################################################################
# <Child model의 Data read>
# DATA는 공공데이터로 제공되는 '골목상권분석정보' 사용
...
#####################################################################
## 점포당 월평균_매출액 상위 30%는 1로 라벨링 수행(Child Model's 종속변수)
#####################################################################
...
# PART3 :데이터 통합마트 생성 및 학습 준비  #########################################################################
##################################################################################################################
# 1) 정제된 데이터들을 통합하여 마트를 생성
# 2) Train Set과 Test set으로 데이터 분리
# 3) 통합마트의 데이터의 결측치 처리
# 4) Min-Max scaling
...
# PART4 : 모델 학습 및 비교 #######################################################################################
##################################################################################################################
## 3가지 방식으로 분류모델을 학습하고 모델성능과 학습속도를 비교한다.
## 
## Model 1 : Basic Model
### RandomForestClassifier & 하이퍼파라메터 튜닝 없음
##
## Model 2 : GridSearchCV Model
### RandomForestClassifier & GridSearchCV기반 하이퍼파라메터 튜닝
##
## Model 3 : MAB 기반 학습 Model
### RandomForestClassifier & MAB Model기반 하이퍼파라메터 튜닝
...
# 모델 저장
joblib.dump(gscv_rfc, './model2.pkl')
...
#################################### Model 2 : GridSearchCV Model END ######################################
...
# 모델 저장
joblib.dump(best_rfc, './model3.pkl')

####################################### Model 3 : MAB 기반 학습 Model #######################################


####################################################################################################################
####################################### PART4 : 모델 학습 및 비교 END ###############################################
####################################################################################################################
